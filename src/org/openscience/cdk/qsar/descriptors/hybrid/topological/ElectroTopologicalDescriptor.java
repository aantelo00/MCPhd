...
public class ElectroTopologicalDescriptor implements IHybridDescriptor {
	...
	public DescriptorValue calculate(IAtomContainer atomcontainer) {
		...
		double total = updateElectroTopologicValues();
		this.container.setProperty(IHybridDescriptor.Descriptor.TOTALELECTROTOPOLOGICAL, total);
	...
	private double updateElectroTopologicValues() {
		...
		for (int i = 0; i < nAtomos; i++) {
			...
			if(!atomoOrigen.getSymbol().equals("H")){
				...
				atomoOrigen.setProperty(IHybridDescriptor.Descriptor.ELECTROTOPOLOGICAL, valorEstateIntAtomoOrigen + efecto);
