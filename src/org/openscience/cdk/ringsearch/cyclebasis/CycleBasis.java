...
public class CycleBasis {
	...
	public CycleBasis (UndirectedGraph g) {
		...
		Iterator it = g.edgeSet().iterator(); it.hasNext();) {
			...
			if (edges.size() > 1) {
				...
				Edge minEdge = edge;
				for (@SuppressWarnings("rawtypes")
				Iterator jt = edges.iterator(); jt.hasNext();) {
		...
		for (@SuppressWarnings("rawtypes")
		Iterator it = biconnectedComponents.iterator(); it.hasNext();) {
			@SuppressWarnings("rawtypes")
			...
			if (edges.size() > 1) {
				...
				for (@SuppressWarnings("rawtypes")
				Iterator edgeIt = edges.iterator(); edgeIt.hasNext();) {
