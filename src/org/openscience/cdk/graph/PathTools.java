...
//					logger.debug("wie oft???");
    ...
    /**
     * Esta es una variante, todos los caminos pero no pueden tener un heteroatomo
     */
    public static List<List<IAtom>> getAllPathsCalede(IAtomContainer atomContainer, IAtom start, IAtom end) {
        allPaths = new ArrayList<List<IAtom>>();
        if (start.equals(end)) return allPaths;
        findPathBetweenCalede(atomContainer, start, end, new ArrayList<IAtom>());
        return allPaths;
    }

    private static void findPathBetweenCalede(IAtomContainer atomContainer, IAtom start, IAtom end, List<IAtom> path) {
        if (start == end) {
            path.add(start);
            allPaths.add(new ArrayList<IAtom>(path));
            path.remove(path.size() - 1);
        ...
        List<IAtom> nbrs = atomContainer.getConnectedAtomsList(start);
        for (IAtom nbr : nbrs) {
        	if(nbr.getSymbol().equals("C") || nbr == end)
        	 findPathBetweenCalede(atomContainer, nbr, end, path);
//        	else{
